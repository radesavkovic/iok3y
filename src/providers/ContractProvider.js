...
export const ContractProvider = ({ children }) => {
  ...
  const busdcontract = new web3Instance2.eth.Contract(erc20Abi, config.busdAddress);
  ...
  const updateInfo = () => setUpdate(!update);

  return (
    <ContractContext.Provider
      value={{ web3, busdcontract, contract, wrongNetwork, getBusdBalance, fromWei, toWei, getBusdApproved, updateInfo, update }}
    >
      {children}
    </ContractContext.Provider>
  );
